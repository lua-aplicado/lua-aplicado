--------------------------------------------------------------------------------
--- Tools CLI/configuration handler
-- @module lua-aplicado.dsl.tools_cli_config
...
local log, dbg, spam, log_error
      = import 'lua-aplicado/log.lua' { 'make_loggers' } (
         "lua-aplicado/dsl/tools_cli_config", "TCC"
       )
...
local dump_nodes
      = import 'lua-nucleo/dsl/dump_nodes.lua'
      {
        'dump_nodes'
      }
...
do
  ...
  load_tools_cli_config = function(
    ...
    if help_printed then
      -- TODO: This is caller's business!
      -- #27
      return os.exit(1)
    end
    ...
    if CONFIG.import == import then
      -- TODO: Hack. Use metatables instead
      -- #29
      CONFIG.import = nil
    end
