--------------------------------------------------------------------------------
--- Routines to handle configuration in DSL format
-- @module lua-aplicado.dsl.config_dsl
...
local log, dbg, spam, log_error = make_loggers("lua-aplicado/dsl/config_dsl", "CDL")
...
local build_config_dsl = function()
  ...
  types:up "cfg:optional_number" (function(self, info, value)
    self:ensure(
        "unexpected type",
  ...
  types:dictionary "cfg:dictionary"
  {
    loadhook = string_to_node;
  }

  types:dictionary "cfg:optional_dictionary"
  {
    optional = true;
    loadhook = string_to_node;
  }
...
do
  -- NOTE: Heavy. Initialize on-demand?
...
local validate_format = function(data_schema, data)
  ...
  local checker = get_data_walkers()
